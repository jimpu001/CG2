...
void CG2Camera::setProjectionMatrix(glm::mat4 proj)
{
...
void CG2Camera::setViewMatrix(glm::mat4 view)
{
	data.view = view;
	data.i_view = glm::inverse(data.view);
	dirty_ubo = true;
...
void CG2Camera::setNearFar(float nearVal, float farVal)
{
	data.near_far_width_height.x = nearVal;
	data.near_far_width_height.y = farVal;
	dirty_ubo = true;
}
