...
void CG2App::init_gl_state()
{
	...
	geo_deer.load("data/models/deer.cg2vd");
	geo_monkeyface.load("data/models/suzanne.cg2vd");
...
{
	geo_deer.destroyGLObjects();
	geo_monkeyface.destroyGLObjects();
}
...
void CG2App::render_one_frame()
{
	...
	glm::mat4 MVP(1.0f);
	MVP = glm::perspective(glm::radians(60.0f),static_cast<float>(framebuffer_size.x)/framebuffer_size.y,0.1f,10.0f)* // projection
	      glm::lookAt(glm::vec3(0.0f,1.0f,2.0f),glm::vec3(0.0f),glm::vec3(0.0f,1.0f,0.0f))* // view
	      glm::translate(glm::vec3(-0.5f,0.0f,0.0f))* // move
	      glm::rotate(static_cast<float>(time_stamp),glm::vec3(0.0f,1.0f,0.0f)); // rotate
	...
	MVP = glm::perspective(glm::radians(60.0f), static_cast<float>(framebuffer_size.x) / framebuffer_size.y, 0.1f, 10.0f)* // projection
		glm::lookAt(glm::vec3(0.0f, 1.0f, 2.0f), glm::vec3(0.0f), glm::vec3(0.0f, 1.0f, 0.0f))* // view
		glm::translate(glm::vec3(0.5f, 0.0f, 0.0f))* // move
		glm::rotate(static_cast<float>(time_stamp*0.5), glm::vec3(0.0f, 1.0f, 0.0f)); // rotate
	glUniformMatrix4fv(0, 1, GL_FALSE, &(MVP[0][0]));
	geo_monkeyface.render();
