...
void CG2Texture::createFrom(const CG2Image& img, GLfloat maxAnisotropy)
{
	...
	glTexImage2D(GL_TEXTURE_2D, 0, internal_format,
	             ...
	             static_cast<GLenum>(format), img.isHDR()?GL_FLOAT:GL_UNSIGNED_BYTE, img.getPixels());
...
void CG2Texture::setCubeMapSideFrom(const CG2Image &img, GLenum side, GLfloat maxAnisotropy)
{
	...
	glTexImage2D(side, 0, internal_format,
	             img.getWidth(), img.getHeight(), 0,
	             format, img.isHDR()?GL_FLOAT:GL_UNSIGNED_BYTE, img.getPixels());
