                            ...
                            int h)
{
	...
	app->framebuffer_size = glm::ivec2(w,h);
                              ...
                              int mods)
{
	...
	if(action == GLFW_RELEASE){
		app->key_pressed[key] = false;
	}else {
		app->key_pressed[key] = true;
	}
                                 ...
                                 int mods)
{
	...
	if(action == GLFW_RELEASE){
		app->button_pressed[button] = false;
	}else {
		app->button_pressed[button] = true;
	}
                            ...
                            double y)
{
	...
	app->mouse_cursor_delta = cp-app->mouse_cursor;
	app->mouse_cursor = cp;
}
...
bool CG2App::init_glfw_backend()
{
	...
	resize_event(w,h);
	framebuffer_size = glm::ivec2(w,h);
	glfwSetCursorPos(m_glfw_win, w/2, h/2);
	this->mouse_cursor = glm::vec2(w/2,h/2);
	this->mouse_cursor_delta = glm::vec2(0,0);
...
{
	delete m_watch[0];delete m_watch[1];
